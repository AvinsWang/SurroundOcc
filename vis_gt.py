...
def voxel2points(voxel, voxelSize, range=[-40.0, -40.0, -1.0, 40.0, 40.0, 5.4], ignore_labels=[17, 255]):
    ...
    # for neofusion
    points = torch.cat((occIdx[1][:, None] * voxelSize[1] + voxelSize[1] / 2 + range[0], \
                        occIdx[0][:, None] * voxelSize[0] + voxelSize[0] / 2 + range[1], \
                        occIdx[2][:, None] * voxelSize[2] + voxelSize[2] / 2 + range[2]), dim=1)
...
def show_point_cloud(points: np.ndarray, colors=True, points_colors=None, obj_bboxes=None, voxelize=False, bbox_corners=None, linesets=None, ego_pcd=None, scene_idx=0, frame_idx=0, large_voxel=True, voxel_size=0.4) -> None:
    ...
    # ego_points_colors = np.full((12000, 3), [0.5, 0.5, 0.5])
    # ego_pcd.colors = o3d.utility.Vector3dVector(ego_points_colors[:, :3])
    # ego_voxelGrid = o3d.geometry.VoxelGrid.create_from_point_cloud(ego_pcd, voxel_size=0.1)
    # vis.add_geometry(ego_voxelGrid)
...
def vis_nuscene():
    ...
    point_cloud_range = [-51.2, -51.2, -6.4, 51.2, 51.2, 6.4]
    # point_cloud_range = [-51.2, -51.2, -5, 51.2, 51.2, 3]
    # point_cloud_range = [-58, -41.6, -3.0, 70, 41.6, 9.8]
    vis_voxel_size = 0.8

    ignore_labels = [0, 17, 255]


    # If you want to vis the data file provided in GitHub, use the code below
    file = "labels_binary_sweep2.npz" # change this to the file path on your machine
    data = np.load(file)
    ...
    # file = 'labels_binary_sweep2.npz' # change this to the gt folder path on your machine
    # file = 'gts-neolix/labels_binary_sweep20.npz'
    # data = np.load(file)
    # semantics, mask_lidar, mask_camera = data['semantics'], data['mask_lidar'], data['mask_camera']
